---
title: Bagaimana Cara Mengatur Customer Survey per Channel
canonical_url: https://help-center.qontak.com/hc/id/articles/27665233541913-Bagaimana-Cara-Mengatur-Customer-Survey-per-Channel
article_type: task
solvability_type: tool
products:
- Qontak Omnichannel
product_surface: web
language: id
intent_tags:
- customer-feedback-report
- configure-customer-survey-per-channel
- customer-support-ticketin
query_examples:
- Cara Mengatur Customer Survey per Channel
- Bagaimana cara Mengatur Customer Survey per Channel?
- Langkah-langkah Mengatur Customer Survey per Channel di Qontak Omnichannel
- How do I Mengatur Customer Survey per Channel?
- Mau Mengatur Customer Survey per Channel, caranya gimana?
compliance_sensitive: false
plan_scope: null
chunk_groups: []
related_chunks: []
last_verified: null
verified_by: AI:claude-haiku-4-5-20251001
faithfulness_threshold: 0.6
---

## Prerequisites  <!-- confidence:high ✓ -->

Untuk mengatur Customer Survey per Channel di Mekari Qontak Omnichannel, Anda membutuhkan:

• Akun Qontak Omnichannel yang aktif dan terverifikasi
• Akses ke menu Settings dengan role yang memiliki izin mengubah pengaturan Customer Survey
• Minimal satu channel komunikasi yang sudah terintegrasi (WhatsApp, Email, Live Chat, atau channel lainnya)
• Pemahaman dasar tentang jenis survei yang tersedia (CSAT, CES, NPS)

Anda tidak perlu mengaktifkan semua jenis survei sekaligus — dapat memilih sesuai kebutuhan divisi.

## Steps  <!-- confidence:high ✓ -->

1. Masuk ke akun Qontak Omnichannel Anda dan arahkan ke menu **Settings**
   → Halaman Settings akan terbuka dengan berbagai opsi konfigurasi

2. Klik tab **Customer survey**
   → Sistem akan menampilkan tiga jenis survei: CSAT, CES, dan NPS dengan toggle untuk mengaktifkan masing-masing

3. Centang checkbox untuk mengaktifkan jenis survei yang Anda inginkan (dapat memilih lebih dari satu)
   → Toggle akan berubah menjadi aktif (berwarna)

4. Klik **ikon segitiga** pada survei yang dipilih untuk mengatur channel
   → Opsi Select all channels dan Select channel akan muncul

5. Pilih **Select all channels** untuk semua channel, atau **Select channel** untuk channel tertentu
   → Jika memilih Select channel, daftar channel yang tersedia akan ditampilkan

6. Centang checkbox pada channel yang diinginkan (gunakan kolom pencarian jika perlu), lalu klik **Done**
   → Channel yang dipilih akan disimpan untuk survei tersebut

7. Isi kolom **Question** dengan pertanyaan survei (minimum 20 karakter)
   → Pertanyaan akan disimpan sebagai konten utama survei

8. Pilih jenis jawaban berdasarkan emoji yang tersedia sesuai tipe survei (CSAT menggunakan emoji smiley, CES menggunakan skala, NPS menggunakan angka)
   → Format jawaban akan ditampilkan dalam pratinjau

9. (Opsional) Aktifkan toggle untuk menambahkan pertanyaan lanjutan setelah customer memberikan penilaian
   → Opsi untuk pertanyaan lanjutan global atau per penilaian akan muncul

10. Periksa tampilan survei dalam pratinjau di sebelah kanan halaman
    → Pratinjau akan menunjukkan bagaimana customer akan melihat survei

11. Klik **Simpan** atau tombol konfirmasi untuk menyimpan konfigurasi survei
    → Sistem akan menampilkan notifikasi sukses dan survei siap dikirim ke channel yang dipilih

## Expected Result  <!-- confidence:high ✓ -->

Setelah berhasil mengatur Customer Survey per Channel, Anda akan melihat:

• Tab Customer survey menampilkan survei yang telah diaktifkan dengan status toggle aktif
• Channel yang dipilih tercermin dalam pengaturan masing-masing survei (All channels atau daftar channel spesifik)
• Pertanyaan dan format jawaban tersimpan dengan benar sesuai tipe survei
• Pratinjau menunjukkan tampilan akhir survei yang akan diterima customer
• Survei siap didistribusikan ke pelanggan melalui channel yang telah dikonfigurasi
• Data respons dapat dilihat nanti di menu Reports (CSAT, CES, atau NPS sesuai jenis survei)

## Error States  <!-- confidence:medium ~ -->

Kesalahan umum dan cara mengatasinya:

• **Pertanyaan kurang dari 20 karakter**: Sistem akan menampilkan pesan error — panjang pertanyaan minimal 20 karakter. Tambahkan lebih banyak detail pada pertanyaan Anda.

• **Tidak ada channel yang dipilih**: Jika Anda klik Select channel tetapi tidak mencentang channel apapun, survei tidak akan tersimpan. Pastikan minimal satu channel tercentang sebelum klik Done.

• **Toggle tidak bisa diaktifkan**: Verifikasi bahwa akun Anda memiliki role dengan izin mengatur Customer Survey. Hubungi admin account jika perlu izin tambahan.

• **Pratinjau tidak muncul**: Refresh halaman Settings atau logout-login kembali untuk memuat ulang interface.

## Escalation  <!-- confidence:medium ~ -->

Hubungi tim support Mekari Qontak jika Anda mengalami:

• Toggle Customer Survey tidak bisa diaktifkan meski sudah masuk dengan role admin
• Perubahan konfigurasi survei tidak tersimpan setelah klik tombol konfirmasi
• Channel tidak muncul di daftar Select channel meski sudah terintegrasi
• Pratinjau survei tidak menampilkan pertanyaan yang sudah diinput
• Error message yang tidak jelas saat menyimpan konfigurasi

Siapkan informasi berikut sebelum menghubungi support:
• ID akun Omnichannel Anda
• Screenshot halaman Customer survey dan pesan error (jika ada)
• Daftar channel yang sudah terintegrasi
• Langkah spesifik yang dilakukan sebelum error terjadi