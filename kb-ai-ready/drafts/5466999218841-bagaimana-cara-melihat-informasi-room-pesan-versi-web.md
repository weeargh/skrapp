---
title: Bagaimana Cara Melihat Informasi Room Pesan Versi Web
canonical_url: https://help-center.qontak.com/hc/id/articles/5466999218841-Bagaimana-Cara-Melihat-Informasi-Room-Pesan-Versi-Web
article_type: task
solvability_type: tool
products:
- Qontak Omnichannel
product_surface: web
language: id
intent_tags:
- conversation-log-history
- view-informasi-room-pesan
- conversation-management
query_examples:
- Cara Melihat Informasi Room Pesan Versi Web
- Bagaimana cara Melihat Informasi Room Pesan Versi Web?
- Langkah-langkah Melihat Informasi Room Pesan Versi Web di Qontak Omnichannel
- How do I Melihat Informasi Room Pesan Versi Web?
- Mau Melihat Informasi Room Pesan Versi Web, caranya gimana?
compliance_sensitive: false
plan_scope: null
chunk_groups: []
related_chunks: []
last_verified: null
verified_by: AI:claude-haiku-4-5-20251001
faithfulness_threshold: 0.6
---

## Prerequisites  <!-- confidence:high ✓ -->

Untuk melihat informasi room pesan di Qontak Omnichannel versi web, Anda memerlukan:

- Akun Qontak Omnichannel yang aktif
- Role Customer Service, Agent, atau Supervisor
- Akses ke dashboard Qontak melalui web browser
- Minimal satu pesan masuk di Inbox
- Koneksi internet yang stabil

## Steps  <!-- confidence:high ✓ -->

1. Masuk ke akun Omnichannel Qontak melalui web browser
2. Klik menu **Inbox** di navigasi utama — Halaman Inbox akan ditampilkan dengan daftar room pesan (Room List)
3. Amati informasi pada setiap room: Nama customer, Foto Profile, Last Aktif, Channel Customer, Level Status Room, dan jumlah pesan
4. Untuk memfilter room, klik tombol **Filter** dan masukkan format nomor 6289127xxxx
5. Untuk mencari room, klik tombol **Search** dan ketikkan nama atau kata kunci
6. Untuk melihat pesan sebelumnya, klik tombol **Load older conversations** — Sistem akan memuat riwayat pesan lebih lama
7. Perhatikan 4 status pesan: All, Unassigned, Assigned, dan Resolved
8. Untuk menyelesaikan obrolan kadaluarsa, klik **Resolve all expired chat room**

> Screenshot: 21.png
> Image: https://help-center.qontak.com/hc/article_attachments/56527490168601

> Screenshot: 22.png
> Image: https://help-center.qontak.com/hc/article_attachments/56527571471001

> Screenshot: 23.png
> Image: https://help-center.qontak.com/hc/article_attachments/56527490175385

## Expected Result  <!-- confidence:high ✓ -->

Setelah mengikuti langkah-langkah di atas, Anda akan berhasil melihat informasi room pesan dengan hasil berikut:

- Halaman Inbox menampilkan daftar lengkap room pesan dari semua customer
- Setiap room menunjukkan nama, foto profil, status aktif terakhir, channel, level status, dan jumlah pesan
- Fitur Filter, Search, dan Resolve all expired chat room tersedia dan siap digunakan
- Riwayat pesan lama dapat dimuat dengan klik tombol **Load older conversations**

## Error States  <!-- confidence:high ✓ -->

Kondisi umum yang dapat dihadapi:

**Pesan sudah di-resolve tidak dapat dibuka kembali** — Untuk membuka kembali room yang sudah di-resolve, customer harus mengirimkan chat baru kepada Anda, atau Anda dapat mengirimkan broadcast terlebih dahulu dan menunggu customer membalas broadcast tersebut.

**Jika Inbox kosong** — Pastikan minimal ada satu pesan masuk dari customer sebelum melihat informasi room pesan.

Untuk masalah teknis lainnya, hubungi Qontak Support.

## Escalation  <!-- confidence:medium ~ -->

Hubungi Qontak Support jika mengalami:

- Room pesan tidak muncul di Inbox meskipun ada pesan masuk
- Fitur Filter, Search, atau Load older conversations tidak berfungsi
- Tidak dapat mengakses menu Inbox
- Error atau pesan kesalahan saat membuka halaman Inbox

Sediakan informasi berikut saat menghubungi support: screenshot halaman Inbox, account ID, daftar pesan atau channel yang bermasalah, dan waktu kejadian masalah.