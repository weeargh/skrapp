---
title: Sekilas tentang Mobile Qontak Chat dan CRM
canonical_url: https://help-center.qontak.com/hc/id/articles/53617578773529-Sekilas-tentang-Mobile-Qontak-Chat-dan-CRM
article_type: concept
solvability_type: content
products:
- Qontak CRM
- Qontak Omnichannel
- Qontak Chat
product_surface: mobile
language: id
intent_tags:
- product-catalog
- operation-workflow-automa
query_examples:
- Apa itu Mobile Qontak Chat dan CRM?
- Apa fungsi Mobile Qontak Chat dan CRM di Qontak CRM?
- Penjelasan Mobile Qontak Chat dan CRM
- What is Mobile Qontak Chat dan CRM?
- Bagaimana cara kerja Mobile Qontak Chat dan CRM?
compliance_sensitive: false
plan_scope: null
chunk_groups: []
related_chunks: []
last_verified: null
verified_by: AI:claude-haiku-4-5-20251001
faithfulness_threshold: 0.3
---

## Definition  <!-- confidence:high ✓ -->

Mobile Qontak Chat dan Qontak CRM adalah dua aplikasi mobile terpisah yang dirancang untuk mendukung operasional bisnis Anda. Mobile Qontak Chat berfungsi mengelola komunikasi pelanggan dari berbagai channel (WhatsApp, Instagram, Facebook, Live Chat, dan Commerce Channel) dalam satu inbox terpusat. Mobile Qontak CRM berfungsi mengelola data pelanggan, pipeline penjualan, dan tugas tim secara real-time. Kedua aplikasi tersedia untuk iOS dan Android, memungkinkan tim Anda bekerja dengan lebih cepat dan efisien dari perangkat mobile.

> Screenshot: Screenshot
> Image: https://help-center.qontak.com/hc/article_attachments/53617557135897

## Why It Matters  <!-- confidence:medium ~ -->

Memahami perbedaan Mobile Qontak Chat dan Qontak CRM membantu Anda memilih alat yang tepat untuk setiap tugas operasional. Mobile Qontak Chat meningkatkan respons pelanggan dengan mengelola semua pesan dalam satu platform, sementara Mobile Qontak CRM memberikan visibilitas penuh terhadap data pelanggan dan pipeline penjualan. Dengan kedua aplikasi ini, tim Anda dapat memberikan layanan pelanggan yang lebih responsif dan mengelola penjualan lebih efektif dari mana saja.

## Key Attributes  <!-- confidence:medium ~ -->

• Mobile Qontak Chat: Manajemen inbox multi-channel, pemantauan status pesan, assign chat ke anggota tim, respons real-time
• Mobile Qontak CRM: Manajemen kontak dan perusahaan, deal management, task management, dashboard monitoring tim
• Kedua aplikasi: Akses mobile (iOS/Android), sinkronisasi data real-time, desain yang dioptimalkan untuk mobilitas tim
• Integrasi: Mobile Qontak Chat terhubung dengan Omnichannel Qontak; Mobile Qontak CRM terhubung dengan platform CRM Qontak

## Related Tasks  <!-- confidence:low ? -->

• Cara Menggunakan Mobile Qontak Chat untuk Mengelola Percakapan Pelanggan
• Panduan Assign Chat ke Tim di Mobile Qontak Chat
• Cara Memantau Status Pesan di Mobile Qontak Chat
• Mengelola Kontak Pelanggan di Mobile Qontak CRM
• Membuat dan Melacak Deal di Mobile Qontak CRM
• Mengelola Tugas Tim dengan Mobile Qontak CRM
• Mengakses Dashboard Mobile Qontak CRM